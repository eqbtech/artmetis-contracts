...
import {Pubkey} from "../Utils/Pubkey.sol";
...
contract MockLocking is Ownable, RateLimiter, ILocking {
    ...
    // locking values
    mapping(address => uint256) public totalLocking;
    ...
    mapping(uint64 => bool) internal requests;
    ...
    modifier OnlyValidatorOwner(address validator) {
        ...
        //require(owner == msg.sender, "not vailid owner");
        _;
    }
    ...
    modifier ConsensusGuard(uint64 id) {
        ...
        require(!requests[id], "ConsensusReentrantCall");
        requests[id] = true;
        _;
    }
    ...
    function create(
        ...
        require(owners[validator] == address(0), "DuplicateValidator");
        require(
    ...
    function testAddGoat(address _user, uint256 amount) external {
        IERC20(goatToken).safeTransferFrom(msg.sender, address(this), amount);
        unclaimed[_user] += amount;
    }
    ...
    function setTokenWeight(
        ...
        require(tokens[token].exist, "TokenNotFound");
